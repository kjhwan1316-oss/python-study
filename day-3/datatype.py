#리스트
subway=["아이유","변우석","박지훈","유재석"]
print(subway)

subway.append("장원영")
print(subway)

subway.insert(1,"카리나")
print(subway)

print(subway.index("박지훈")) #위치

print(subway.pop()) #뒤 자료 삭제
print(subway)   

name=subway.pop(1)
print(name)

subway.remove("유재석")
print(subway)

subway.append("아이유")
print(subway)

print(subway.count("아이유")) #개수

subway.remove("아이유")
print(subway)

subway.remove("아이유")
print(subway)

num_list=[2,4,5,1,3]
print(num_list)

num_list.sort()
print(num_list)

num_list.reverse()
print(num_list)

num_list.clear()
print(num_list)
#리스트는 가변적안 움직임이 가능하다
#append: 추가, insert: 특정 위치 추가, pop: 특정 위치 삭제, remove: 값 삭제, clear: 모두 제거, sort: 순서대로 정렬, reverse: 반대로 정렬

#튜플: 순서있음, 나열형, 변경불가
menu=("김밥", "오뎅")
print(menu)
#menu[1] = "피자" #변경이 불가능해 오류 발생

(name,age,addr) = ("이순신", 30,"안산")
print(name,age,addr)

#딕셔너리: 키와 값이 쌍으로 구성
classroom = {407:"개발자과정", 402:"영상과정"}
print(classroom)
print(classroom[407])
#print(classroom[404]) #오류
print(classroom.get(407))
print(classroom.get(404)) #None

print(classroom.keys()) #키
print(classroom.values()) #값
print(classroom.items()) #키:값

del classroom[402] #del: 전반적으로 통용되는 삭제
print(classroom)

set